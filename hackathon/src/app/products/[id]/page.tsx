...
const getProductsDetail=(id:number|string)=>
 ...
 export default function Page({ params }: { params: { id: string } }) 
            ...
            <div className="flex  flex-wrap py-10 mt-16"> 
            {
              result.map((product)=>(
                 <div key={product.id}    className="flex justify-between gap-6 pl-20">
                       ...
                       <div>
                             <Image src={product.image} alt={product.name} height={400} width={500}/>          
                        </div>
                 <div className="pt-6">   
                         <div >
                               <h1 className="   pb-4  text-2*1">{product.name}</h1>
                              <h2 className="text-gray-400 font-semibold text-base pb-4">{product.discription}</h2>
                         </div>
                        ...
                        <div className="flex  gap-x-4 pb-4">
                         {
                          sizes.map((item)=>{
                                  return(<div className='h-6 w-6 rounded-full border hover:shadow-xl duration-300 mt-2 flex justify-center items-center'>
                                  <span className="text-sm font-semibold text-center text-gray-600">{item}</span>   
                                  </div>
                            )
                             }
                           )
                      ...
                      <div className=' gap-x-4 items-center mt-6 flex'>
   ...
   
   {
     result.map((product) => (
          <>
            <div className="flex gap-20 items-start mt-10 border-t border-black gap-x-36 ">
                 <span className="text-s mt-4 font-semibold pt-4 ">Product Detail:</span>
                 <p className="text-sm text-gray-400 pt-4 flex-1">{product.productdetail}</p>
              </div>
